...
jQuery(document).ready(($) => {
    $('#connectToServerBtn').click(() => {
        ...
        socket.on('usersList', users => {
            ...
            users.forEach((currUsername) => {
                ...
                $('#usersList').append(`<option value='${currUsername}'>${currUsername}</option>`)
            })
        })
        ...
        if ('type' in data) {
            switch (data.type) {
                case 'File':
                    ...
                    break;
                default:
                    dispalyErr('not handled case ' + data.type)
            }
        }
    ...
    function handleReceivedFile(fileInfo) {
        if ('fileDesc' in fileInfo) {
            receivedFileMetaData = fileInfo.fileDesc
        } else if ('fileData' in fileInfo) {
            receivedFileData[fileInfo.fileData.chunkNumber] = fileInfo.fileData.blob
    ...
    function fileReceiveEnded() {
        ...
        linkToDownload.innerText = 'Click Here To Download ' + receivedFileMetaData.name
